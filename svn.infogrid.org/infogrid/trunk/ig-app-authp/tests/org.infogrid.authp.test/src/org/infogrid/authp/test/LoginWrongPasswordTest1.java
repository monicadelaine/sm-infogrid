...
/**
 * Tests that one cannot log in with the wrong password.
 */
public class LoginWrongPasswordTest1
        extends
            AbstractAuthpTest
{
    /**
    ...
    public void run()
            throws
                Exception
    {
        log.info( "Submitting invalid password to front page" );

        Map<String,String> userPassMap = new HashMap<String,String>();
        userPassMap.put( "lid",            VALID_USER ); // valid user
        userPassMap.put( "lid-credential", "invalid" );  // invalid credential
        ...
        checkRegex( ".*<script.*[Ee]rror.*", DEFAULT_HTTP_REGEX_FLAGS, r.getContentAsString(), "Login form or error message not displayed" );
    ...
    public static void main(
             String [] args )
    {
        LoginWrongPasswordTest1 test = null;
        try {
            if( args.length != 0 ) {
            ...
            test = new LoginWrongPasswordTest1( args );
            test.run();
     ...
     */
    public LoginWrongPasswordTest1(
            String [] args )
        throws
            Exception
    {
