...
/**
 * Same as {@link SqlStoreTest1 SqlStoreTest1}, but with an EncryptedStore in between.
 */
public class EncryptedSqlStoreTest1
        extends
            SqlStoreTest1
{
    /**
    ...
    public static void main(
            String [] args )
    {
        EncryptedSqlStoreTest1 test = null;
        try {
            if( args.length != 2 ) {
            ...
            test = new EncryptedSqlStoreTest1( args );
            test.run();
     ...
     *
     * @param args command-line arguments
     * @throws Exception all sorts of things may go wrong during tests
     */
    public EncryptedSqlStoreTest1(
            String [] args )
        throws
            Exception
    {
        super( args[0], EncryptedSqlStoreTest1.class );
    ...
    // Our Logger
    private static Log log = Log.getLogInstance( EncryptedSqlStoreTest1.class);
}
